...
/*
 * GPU performance tweaks (safe, opt-in via compile-time flags)
 *
 * HOW TO ENABLE:
 *  - Define GPU_PERF_TWEAKS=1 to enable boost helper and optional heuristics.
 *  - Optionally define GPU_BOOST_FREQ_MHZ (numeric) to raise temporary boost freq.
 *  - Optionally define GPU_BOOST_DURATION_MS (numeric) to set boost duration.
 *  - Optionally define GPU_SAFE_MAX_INCREASE_PERCENT (numeric, 0-100) to allow
 *    increasing max allowed frequency ceiling by percentage (default 10).
 *
 * These tweaks are conservative by default and guarded behind checks:
 *  - They will not override thermal governor decisions.
 *  - They are compile-time optional; by default nothing changes behavior.
 */

#ifndef GPU_PERF_TWEAKS
#define GPU_PERF_TWEAKS 0
#endif
...
#ifndef GPU_BOOST_FREQ_MHZ
#define GPU_BOOST_FREQ_MHZ 900   /* fallback boost frequency in MHz (safe default) */
#endif

#ifndef GPU_BOOST_DURATION_MS
#define GPU_BOOST_DURATION_MS 200  /* default boost duration (ms) */
#endif

#ifndef GPU_SAFE_MAX_INCREASE_PERCENT
#define GPU_SAFE_MAX_INCREASE_PERCENT 10 /* allow up to +10% max freq ceiling */
#endif

/* Debug prints for tweak code */
#ifndef GPU_TWEAK_DEBUG
#define GPU_TWEAK_DEBUG 0
#endif

#if GPU_TWEAK_DEBUG
#define TWEAK_DBG(fmt, ...) pr_info("gpufreq_tweak: " fmt, ##__VA_ARGS__)
#else
#define TWEAK_DBG(fmt, ...) do {} while (0)
#endif

/* Simple workqueue-based temporary boost to avoid sleeping in atomic contexts. */
struct gpu_boost_work {
    struct work_struct work;
    unsigned int freq_mhz;
    unsigned int duration_ms;
    /* store whatever handle/reference needed to request freq in your driver */
    void *drv_priv;
};

static void gpu_boost_work_fn(struct work_struct *work)
{
    struct gpu_boost_work *bw = container_of(work, struct gpu_boost_work, work);
    unsigned int target_mhz = bw->freq_mhz;
    unsigned int duration_ms = bw->duration_ms;
    void *priv = bw->drv_priv;

    TWEAK_DBG("boost start: %u MHz for %u ms\n", target_mhz, duration_ms);

    /*
     * IMPORTANT: Replace the following placeholder calls with the actual
     * driver APIs to request frequency. These calls are intentionally
     * wrapped in weak helper functions below so they can be adapted to
     * existing driver symbols without modifying other logic.
     */

    if (gpufreq_tweak_request_freq(priv, target_mhz) == 0) {
        /* sleep in a safe kernel-friendly way */
        msleep(duration_ms);
        gpufreq_tweak_release_freq(priv);
    } else {
        TWEAK_DBG("failed to request boost freq\n");
    }

    kfree(bw);
}

/* Public helper to request a temporary boost. Non-blocking caller. */
static inline int gpu_request_temporary_boost(void *drv_priv,
                                              unsigned int freq_mhz,
                                              unsigned int duration_ms)
{
    struct gpu_boost_work *bw;

    if (!drv_priv) {
        TWEAK_DBG("no drv_priv\n");
        return -EINVAL;
    }
    ...
    INIT_WORK(&bw->work, gpu_boost_work_fn);
    bw->freq_mhz = freq_mhz;
    bw->duration_ms = duration_ms;
    bw->drv_priv = drv_priv;

    /* queue to system workqueue; adjust if driver has dedicated wq */
    queue_work(system_wq, &bw->work);
    return 0;
}

/*
 * Weak helper hooks which should be implemented/aliased in the driver:
 * - gpufreq_tweak_request_freq(priv, freq_mhz): requests freq; return 0 on success
 * - gpufreq_tweak_release_freq(priv): releases the temporary request
 *
 * If the driver already has equivalent functions, provide wrappers or map them.
 * Otherwise, you can implement these to call the driver's existing DVFS APIs.
 */
#ifndef HAVE_GPUFREQ_TWEAK_HOOKS
static __weak int gpufreq_tweak_request_freq(void *priv, unsigned int freq_mhz)
{
    TWEAK_DBG("gpufreq_tweak_request_freq weak stub called\n");
    return -ENOSYS;
}
static __weak void gpufreq_tweak_release_freq(void *priv)
{
    TWEAK_DBG("gpufreq_tweak_release_freq weak stub called\n");
}
#endif /* HAVE_GPUFREQ_TWEAK_HOOKS */

/* Increase the driver's maximum allowed frequency ceiling safely at init time
 * if GPU_SAFE_MAX_INCREASE_PERCENT > 0 and CONFIG_OPTION_ENABLE_MAX_RAISE is defined
 * This is conservative: only adjusts the ceiling by a small percent.
 * The driver must expose gpufreq_driver_get_max_mhz and gpufreq_driver_set_max_mhz,
 * which can be implemented as wrappers to existing driver APIs.
 */
static inline void gpufreq_safe_raise_max_ceiling(void *drv_priv)
{
#if defined(CONFIG_GPUFREQ_ALLOW_MAX_RAISE)
    unsigned int cur_max = gpufreq_driver_get_max_mhz(drv_priv);
    unsigned int increase = (cur_max * GPU_SAFE_MAX_INCREASE_PERCENT) / 100;
    unsigned int new_max = cur_max + increase;

    if (new_max > cur_max) {
        TWEAK_DBG("raising max ceiling %u -> %u MHz\n", cur_max, new_max);
        gpufreq_driver_set_max_mhz(drv_priv, new_max);
    }
#else
    TWEAK_DBG("max raise disabled at build time\n");
#endif
}

#endif /* GPU_PERF_TWEAKS */
